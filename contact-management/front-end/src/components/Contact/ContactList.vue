...
async function fetchContacts() {
  const res = await contactList(token.value, search);
  const data = await res.json();

  if (res.status === 200) {
    contacts.value = data.data;
    totalPage.value = data.paging.total_page;
  } else {
    await alertError("Contact list fetch failed");
  }
}
...
<template>
  <Header title="Contact List" icon="users" />
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    ...
    <!-- Contact Card 1 -->
    <CardContact :contacts="contacts" :handleDelete="handleDelete" />
  </div>
  ...
  <div class="mt-10 flex justify-center">
    ...
    >
      <a
        @click="handleChangePage(search.page - 1)"
        v-if="search.page > 1"
